...
To install every repository skill together in ChatGPT/Codex:

```bash
codex plugin marketplace add vladicaster/agent-skills --ref main
```

Install **Vladicaster Agent Skills** from **Vladicaster Tools** in the plugin browser. Update it with:
...
Enable auto-update under `/plugin` → **Marketplaces** → **vladicaster-tools**, or update manually with:
...
A new skill included in a later release becomes available after this host-specific upgrade and reload. Standalone installations remain unchanged. See the [complete plugin instructions](../../docs/plugin-installation-and-updates.md).

## Updating this skill

Copied standalone installations are snapshots and do not automatically follow source changes. The full plugin follows the host-specific update path above; Claude marketplace auto-update applies only when enabled.

- **ChatGPT Work:** Ask ChatGPT: `Update my installed develop-go-to-market-strategy skill from https://github.com/vladicaster/agent-skills/tree/main/product/develop-go-to-market-strategy`. The update should retrieve and validate the complete directory, report meaningful changes or local conflicts, and replace the installed copy. Refresh or reopen the Skills page if necessary.
